...
<section class="update container text-center">
    <div class="card">
        ...
        <div class="row">
            <div class="col-12">
                @if(auth()->guard('web')->user()->image)
                    <img src="{{asset(auth()->guard('web')->user()->image)}} " alt="" width="100">
                @else
        ...
        <form method="post" action="{{route('profile.user',auth()->guard('web')->user()->id)}}">
            @csrf
            <div class="form-group text-right">
                <label for="Name" class="text-light-green">الاسم</label>
                {!!  Form::text('name',auth()->guard('web')->user()->name,[
                'class'=>'form-control ',
                ]) !!}
            </div>
            ...
            <div class="form-group text-right">
                ...
                {!!  Form::text('email',auth()->guard('web')->user()->email,[
                 'class'=>'form-control ',
                ]) !!}
            </div>
